...
Shader discardShader("discard & cullface");
...
{
	discardShader.init();
	discardShader.attach(GL_VERTEX_SHADER, "discard.vert");
	discardShader.attach(GL_FRAGMENT_SHADER, "discard.frag");
	discardShader.link();
	discardShader.use();
...
void init(void)
{
    ...
    glEnable(GL_DEPTH_TEST| GL_CULL_FACE); // GL_DEPTH_TEST	
	glFrontFace(GL_CCW);
