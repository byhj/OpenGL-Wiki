...
GLuint model_matrix_loc, mvp_matrix_loc, view_matrix_loc;
Shader specularShader("specular");
objMesh mesh;
...
void init_shader()
{
	...
	glClearDepth(1.0f);
	specularShader.init();
	specularShader.attach(GL_VERTEX_SHADER, "smooth-specular.vert");
	specularShader.attach(GL_FRAGMENT_SHADER, "smooth-specular.frag");
	specularShader.link();
	specularShader.use();
	program = specularShader.program;
...
void render()
{
	...
	glUniformMatrix4fv(model_matrix_loc, 1, GL_FALSE, &view_matrix[0][0]);
	mesh.render();	
	glutSwapBuffers();
	glutPostRedisplay();
}
